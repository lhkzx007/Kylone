...
public class SeekView extends View {
    ...
    private void drawProgress(Canvas canvas) {
        ...
        if (mBufferDrawable != null && mBufferProgress >= 0) {
            int p = (int) ((float) mBufferProgress / 100 * mMax);
            drawProgressDrawable(canvas, bounds, 0, p, mBufferDrawable);
        }
    ...
    private void init() {
        ...
        mBufferDrawable = new ColorDrawable(0xFF999999);
     ...
     */
    public void setDrawable(Drawable backDrawable, Drawable progressDrawable, Drawable changedDrawable, Drawable bufferDrawable) {
        mBackProgressDrawable = backDrawable;
        mProgressDrawable = progressDrawable;
        mChangedDrawable = changedDrawable;
        mBufferDrawable = bufferDrawable;
    }
    ...
    public synchronized void setBufferProgress(int bufferProgress) {
        if (bufferProgress < 0) {
            bufferProgress = 0;
        }
        if (bufferProgress > mMax) {
            bufferProgress = mMax;
        }
        if (bufferProgress != mBufferProgress) {
